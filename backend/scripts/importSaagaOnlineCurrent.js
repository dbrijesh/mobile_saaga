...
function normalizeSubcategory(name) {
  ...
  if (normalized.toLowerCase() === 'juice') normalized = 'Juice';
...
async function importSaagaOnline() {
  try {
    console.log('📊 Reading Saaga_Online.xlsx...\n');

    const workbook = XLSX.readFile(path.join(__dirname, '../../Saaga_Online.xlsx'));
    ...
    const rawData = XLSX.utils.sheet_to_json(worksheet, {defval: '', range: 1});
