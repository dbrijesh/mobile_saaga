...
/**
 * Normalize category name with specific mappings
 */
function normalizeCategory(name) {
  ...
  if (normalized.toLowerCase().includes('milk') && normalized.toLowerCase().includes('cereal')) {
    return 'Milk, Cereals & Nuts';
  }
  if (normalized.toLowerCase().includes('bakery') && normalized.toLowerCase().includes('biscuit')) {
    return 'Bakery & Biscuits & Juice';
  }
...
/**
 * Normalize subcategory name with specific mappings
 */
function normalizeSubcategory(name) {
  if (!name) return '';
  let normalized = normalizeName(name);

  // Map to existing subcategories
  if (normalized.toLowerCase() === 'milk') return 'Milk';
  if (normalized.toLowerCase().includes('oats') && normalized.toLowerCase().includes('date')) {
    return 'Oats & Dates';
  }
  if (normalized.toLowerCase().includes('vermicili') || normalized.toLowerCase().includes('poha')) {
    return 'Vermicelli & Poha';
  }
  if (normalized.toLowerCase().includes('bakery') && normalized.toLowerCase().includes('biscuit')) {
 ...
 */
async function importSaagaOnline1() {
  try {
    console.log('📊 Reading Saaga-Online-1.xlsx...\n');

    const workbook = XLSX.readFile(path.join(__dirname, '../../Saaga-Online-1.xlsx'));
    const worksheet = workbook.Sheets['MILK CEREAL NUTS '];
    ...
    const categoryNames = [...new Set(rawData.map(row => normalizeCategory(row['CATEGORY '])).filter(c => c))];
    const newCategories = [];
    ...
    console.log('\n' + '='.repeat(80));
    console.log('✅ Saaga-Online-1 import completed successfully!');
    ...
    console.error('❌ Error importing Saaga-Online-1 data:', error);
    process.exit(1);
  }
}
